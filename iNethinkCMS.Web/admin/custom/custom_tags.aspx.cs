...
namespace iNethinkCMS.Web.admin.custom
{
    ...
    public partial class custom_tags : Admin_BasePage
    {
        iNethinkCMS.BLL.BLL_iNethinkCMS_Custom_Tags bll = new iNethinkCMS.BLL.BLL_iNethinkCMS_Custom_Tags();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            switch (vAct)
            {
                case "guide":
                    ...
                    if (vID == 0)
                    {
                        this.navInfoID.InnerText = vNavInfo + "自定标签添加";
                        this.Button_Submit.Click += new EventHandler(Button_Submit_Click_Add);
                    }
                    else
                    {
                        this.navInfoID.InnerText = vNavInfo + "自动标签修改";
                ...
                default:
                    this.navInfoID.InnerText = vNavInfo + "自定标签管理";
        ...
        private bool Fun_GetValue()
        {
            ...
            {
                MessageBox.Show(this, "请输入标签名称!");
                return false;
            }
            if (Command.Command_Validate.IsHasCHZN(this.txtName.Text.Trim()))
            {
                MessageBox.Show(this, "标签名称不能含有中文字符!");
                return false;
            }
            if (this.txtRemark.Text.Trim().Length ==0)
            {
                MessageBox.Show(this, "请输入标签备注!");
                return false;
            }

            vName = this.txtName.Text.Trim();
            vRemark = this.txtRemark.Text.Trim();
            vCode = this.txtCode.Text.Trim();
            return true;
        }
        ...
        protected void Button_Submit_Click_Add(object sender, EventArgs e)
        {
            ...
            {
                if (bll.GetRecordCount("Name = '" + vName + "'") > 0)
                {
                    MessageBox.Show(this, "标签名称 [" + vName + "] 已经存在!");
                    return;
                }

                model.Name = vName;
                model.Remark = vRemark;
                model.Code = vCode;
                bll.Add(model);

                bll_upload.UpdateUploadFile(vCode, 2, bll.GetMaxID(), 0);
        ...
        protected void Button_Submit_Click_Edit(object sender, EventArgs e)
        {
            if (Fun_GetValue() == true)
            {
                ...
                if (bll.GetRecordCount("ID <> " + vID + " And Name = '" + vName + "'") > 0)
                {
                    MessageBox.Show(this, "标签名称 [" + vName + "] 已经存在!");
                    return;
                }
                ...
                bll_upload.UpdateUploadFile_Reset(2, vID, 0);  //重置上传表中的数据
                bll_upload.UpdateUploadFile(vCode, 2, vID, 0);
        ...
        protected void Fun_Delete(int byID)
        {
            ...
            {
                bll_upload.UpdateUploadFile_Reset(2, byID, 0);  //重置上传表中的数据
